'''
Modified from https://github.com/brentp/bio-playground/blob/master/igv/igv.py
Python script to save IGV screenshots from BED file locations
'''
...
def bed2igv():
    ...
    work_dir = 'sv_benchmark/input.na12878/'
    bed_file = work_dir + 'lumpy-Mills2012-call-set_no_cr.bed'
    bam_file = 'GIAB12878/mapping/GIAB12878_dedup.bam'
    ...
    with(open(bed_file, 'r')) as bed:
        for line in bed:
            ...
            coord = chrom+':'+str(start-win)+'-'+str(end+win)
            screenshot_name = work_dir + 'png_no_cr/' + chrom+'_'+str(start-win)+'_'+str(end+win)+''

            igv.go(coord)
            igv.sort('position')
            igv.collapse()
            print('Saving file: %s' % screenshot_name)
            igv.save(screenshot_name)
