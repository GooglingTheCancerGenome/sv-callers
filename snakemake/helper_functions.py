...
def make_output():
    ...
    with open(config["samples"], "r") as fp:
        mode = config["mode"]
        assert(mode.startswith("s") or mode.startswith("g")), \
            "Unknown SV calling mode: use either (s)omatic or (g)ermline."

        reader = DictReader(line for line in fp if not line.startswith("#"))
        outfiles = []
        for r in reader:
            for c in get_callers():
                path = os.path.join(r["PATH"], r["TUMOR"])
                if mode.startswith("s") is True:
                    path += "--" + r["NORMAL"]
